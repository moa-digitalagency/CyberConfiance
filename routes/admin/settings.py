 ...
 * Description : Fichier settings.py du projet CyberConfiance
...
def site_settings():
    ...
    if request.method == 'POST':
        try:
            ...
            db.session.rollback()
            log_activity('ADMIN_SETTINGS_UPDATE', f'Erreur mise à jour paramètres: {str(e)}', success=False)
...
def seo_settings():
    ...
    if request.method == 'POST':
        try:
            ...
            db.session.rollback()
            log_activity('ADMIN_SEO_UPDATE', f'Erreur mise à jour SEO: {str(e)}', success=False)
            flash(f'Erreur lors de la mise à jour : {str(e)}', 'danger')
            return redirect(url_for('admin_panel.seo_settings'))
...
def seo_edit(seo_id):
    ...
    if request.method == 'POST':
        try:
            ...
            log_activity('ADMIN_SEO_UPDATE', f'Erreur mise à jour SEO: {str(e)}', success=False)
            flash(f'Erreur lors de la mise à jour : {str(e)}', 'danger')
            return redirect(url_for('admin_panel.seo_edit', seo_id=seo_id))
