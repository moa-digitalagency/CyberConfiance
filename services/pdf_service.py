...
class PDFReportService:
    ...
    def generate_breach_report(self, breach_analysis, breach_result, ip_address):
        ...
        page.insert_text((30, y_pos), "RAPPORT D'ANALYSE DE FUITE DE DONNÉES", 
                        fontsize=18, fontname="helv-bold", color=self.base_color)
        y_pos += 30
        
        page.insert_text((30, y_pos), "Type de rapport: Analyse de sécurité des fuites de données", 
        ...
        page.insert_text((40, y_pos), "INFORMATIONS DE L'ANALYSE", 
                        fontsize=12, fontname="helv-bold")
        y_pos += 20
        ...
        y_pos += 15
        page.insert_text((40, y_pos), "NIVEAU DE RISQUE", fontsize=12, fontname="helv-bold")
        y_pos += 20
        risk_text = (breach_analysis.risk_level or 'Sûr').upper()
        page.insert_text((40, y_pos), risk_text, fontsize=14, fontname="helv-bold", color=risk_color)
        y_pos += 40
        
        if breach_result.get('breaches'):
            page.insert_text((30, y_pos), "DÉTAILS DES FUITES DÉTECTÉES", 
                            fontsize=14, fontname="helv-bold", color=self.danger_color)
            y_pos += 25
            ...
            page.insert_text((40, y_pos), "✓ Aucune fuite détectée", 
                            fontsize=12, fontname="helv-bold", color=self.success_color)
            y_pos += 20
            page.insert_text((40, y_pos), 
    ...
    def generate_security_analysis_report(self, security_analysis, breach_analysis, ip_address):
        ...
        y_pos += 15
        page.insert_text((40, y_pos), "NIVEAU DE MENACE", fontsize=12, fontname="helv-bold")
        y_pos += 20
        threat_text = (security_analysis.threat_level or 'Sûr').upper()
        page.insert_text((40, y_pos), threat_text, fontsize=14, fontname="helv-bold", color=threat_color)
        y_pos += 40
        
        page.insert_text((30, y_pos), "RÉSULTATS DÉTAILLÉS", 
                        fontsize=14, fontname="helv-bold")
        y_pos += 25
        ...
        for label, value, color in stats:
            ...
            page.insert_text((40, y_pos + 15), f"{label}: {value}", fontsize=10, fontname="helv-bold")
            y_pos += 35
        ...
        if breach_analysis:
            ...
            if breach_analysis.breaches_data:
                ...
                if breach_data.get('breaches'):
                    page.insert_text((40, y_pos), "Fuites identifiées:", fontsize=10, fontname="helv-bold")
                    y_pos += 18
